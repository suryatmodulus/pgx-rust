...
impl PartialEq<AnyNumeric> for AnyNumeric {
    ...
    fn eq(&self, other: &AnyNumeric) -> bool {
        unsafe { direct_function_call(pg_sys::numeric_eq, vec![self.as_datum(), other.as_datum()]).unwrap() }
    }

    #[inline]
    fn ne(&self, other: &AnyNumeric) -> bool {
        unsafe { direct_function_call(pg_sys::numeric_ne, vec![self.as_datum(), other.as_datum()]).unwrap() }
    }
}
...
impl PartialOrd for AnyNumeric {
    ...
    fn lt(&self, other: &Self) -> bool {
        unsafe { direct_function_call(pg_sys::numeric_lt, vec![self.as_datum(), other.as_datum()]).unwrap() }
    }

    #[inline]
    fn le(&self, other: &Self) -> bool {
        unsafe { direct_function_call(pg_sys::numeric_le, vec![self.as_datum(), other.as_datum()]).unwrap() }
    }

    #[inline]
    fn gt(&self, other: &Self) -> bool {
        unsafe { direct_function_call(pg_sys::numeric_gt, vec![self.as_datum(), other.as_datum()]).unwrap() }
    }

    #[inline]
    fn ge(&self, other: &Self) -> bool {
        unsafe { direct_function_call(pg_sys::numeric_ge, vec![self.as_datum(), other.as_datum()]).unwrap() }
    }
}
...
impl Ord for AnyNumeric {
    ...
    fn cmp(&self, other: &Self) -> Ordering {
        let cmp: i32 = unsafe {
            direct_function_call(pg_sys::numeric_cmp, vec![self.as_datum(), other.as_datum()]).unwrap()
        };
        if cmp < 0 {
            Ordering::Less
        } else if cmp > 0 {
