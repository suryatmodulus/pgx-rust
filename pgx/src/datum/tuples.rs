    ...
    B: FromDatum + IntoDatum,
{
        ...
        Self: Sized,
    {
        ...
        let a_datum =
            if a.is_some() { A::from_polymorphic_datum(a.unwrap(), false, A::type_oid()) } else { None };

        let b_datum =
            if b.is_some() { B::from_polymorphic_datum(b.unwrap(), false, B::type_oid()) } else { None };
    ...
    C: FromDatum + IntoDatum,
{
        ...
        Self: Sized,
    {
        ...
        let c_datum =
            if c.is_some() { C::from_polymorphic_datum(c.unwrap(), false, C::type_oid()) } else { None };
