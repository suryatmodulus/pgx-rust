...
mod tests {
    ...
    fn elided_extern_is_elided() {
        ...
        let result: bool =
            Spi::get_one("SELECT exists(SELECT 1 FROM pg_proc WHERE proname = 'func_in_diff_schema');")
                .expect("expected result");
        assert_eq!(result, true);

        // Validate that the function we expect not to exist, doesn't
        let result: bool =
            Spi::get_one("SELECT exists(SELECT 1 FROM pg_proc WHERE proname = 'func_elided_from_schema');")
                .expect("expected result");
    ...
    fn elided_type_is_elided() {
        ...
        let result: bool = Spi::get_one("SELECT exists(SELECT 1 FROM pg_type WHERE typname = 'testtype');")
            .expect("expected result");
        assert_eq!(result, true);

        // Validate that the type we expect not to exist, doesn't
        let result: bool = Spi::get_one("SELECT exists(SELECT 1 FROM pg_type WHERE typname = 'elidedtype');")
            .expect("expected result");
        assert_eq!(result, false);
    }
    ...
    fn custom_handwritten_to_sql_type() {
        ...
        let result: bool =
            Spi::get_one("SELECT exists(SELECT 1 FROM pg_type WHERE typname = 'manuallyrenderedtype');")
                .expect("expected result");
        assert_eq!(result, true);
    }
}
